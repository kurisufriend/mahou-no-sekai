...
{
    rows r = sqleasy_q{db, dumbfmt({"select * from boards where name=\"", se(board), "\" limit 1;"})}.exec();
    row data = r.at(0);
...
{
    rows threads = sqleasy_q{db, dumbfmt({"select * from threads where board=\"",se(board),"\" order by bump_time desc;"})}.exec();
    this->board_cache[board] = threads;
}

void mns::gcache::update_thread_index(sqlite3 *db, std::string board, int op)
{
    rows threads = sqleasy_q{db, dumbfmt({"select * from threads where no=",std::to_string(op)," and board=\"",se(board),"\";"})}.exec();
    row thread = !threads.empty() ? threads.at(0) : row();

    rows posts = sqleasy_q{db, dumbfmt({"select * from posts where op=",std::to_string(op)," and board=\"",se(board),"\";"})}.exec();
